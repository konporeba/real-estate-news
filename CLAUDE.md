...
- **Pipeline worker** → plain Node, launched by `npm run collect`. `src/worker/`, `src/lib/collection/`, `src/lib/llm/`, `src/lib/email/`, `src/lib/scheduler/`, `src/lib/visuals/`. Uses Node-oriented dependencies (`rss-parser`, `@anthropic-ai/sdk`, `nodemailer`, `google-auth-library`) and long-running work the edge runtime is wrong for.

Rules, enforced by `no-restricted-imports` in `eslint.config.js` (both directions):

- App code must **not** import `@/lib/collection/*`, `@/lib/llm/*`, `@/lib/email/*`, `@/lib/scheduler/*`, `@/lib/visuals/*`, or `@/worker/*` — it drags Node built-ins into the workerd bundle. If a page needs pipeline results, read them from the database.
