...
- **Decision**: CLOSED 2026-08-01 — re-ran `npm run collect` + `npm run rank` on digest `09956a63` (window 2026-07-27 → 2026-08-02; a thin 2-article pool, the largest the then-current window-tiling checkpoint allowed — see below). Operator logged in via the PIN gate and eyeballed the shortlist detail page live: both clusters rendered with real Spanish→Polish translations (one full title+summary, one title-only because its source had no lede — correctly falls through to no summary paragraph rather than a stale/wrong value). The literal "operator opens a `ready_for_selection` digest and sees real Polish text" path is now confirmed. Two unrelated defects were found and fixed along the way, outside F3's own scope: (1) `PIN_PEPPER` in `.dev.vars` contained a `#`, which the dotenv-style parser silently truncates as a comment marker, so every PIN attempt was verified against a 55-char pepper instead of the real 60-char one — login could never have succeeded until the pepper was rotated to a dotenv-safe charset. (2) The PIN input's client-side `pattern="\d{6}"` plus `autocomplete="one-time-code"` produced a persistent, unexplained native "match the requested format" validation failure even for a correct 6-digit PIN — removed in favor of relying on the existing server-side `pinInputSchema` check, which was already sufficient. Also newly surfaced: window-tiling's `resolveCollectionWindow` (`src/lib/collection/window.ts`) always fetches up to "now" with no upper bound tied to the target digest's own `window_end`, so a one-off re-collect on a stale/old digest silently steals calendar coverage from whatever digest comes next — this is what starved `09956a63`'s pool down to 2 articles. Not fixed here (behavioral, not a defect in this change); worth a follow-up if manual re-triggers on old digests recur.
