...
- **Parallel with:** F-05, S-10
- **Blockers:** —
- **Unknowns:** — (OQ#2 resolved: Canva ruled out, shipped on Google Slides instead; OQ#3 resolved: carousel length driven by story count — see Open Roadmap Questions #2 and #3.)
...
2. ~~**Visual-template (Canva) API access — paid tier, possible access application with external lead time.**~~ — **Resolved 2026-09-08 at S-06 planning: no, ruled out.** Canva's Autofill API requires Enterprise organization membership, even for autofilling a plain design — verified by the operator against their own Pro account. Google Slides gives the same "operator owns the design, code fills named slots" shape for free, needs no access application, and satisfies US-13 literally. S-06 shipped on Google Slides.
3. ~~**Carousel length — fixed, or driven by story count?**~~ — **Resolved 2026-09-08 at S-06 planning: driven by story count.** One cover slide plus one slide per story — length follows content, so there are no filler slides and no silently dropped story.
4. ~~**Selection granularity — pick the cluster or a specific article within it?**~~ — **Resolved at S-04 planning: the cluster is the unit of selection, not an article within it.**
...
- **S-10: operator learns when nothing ran (an external monitor alerts on missing heartbeat) rather than mistaking silence for "no news," and a run missed while the machine was off is executed on next start.** — Archived 2026-09-19 → `context/archive/2026-09-19-ops-heartbeat-and-catchup/`. Lesson: —.
