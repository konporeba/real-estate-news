...
The insight that makes this worth building: the hard part is not the plumbing. Posting to Instagram, Facebook, and LinkedIn is a well-established, mechanical integration against each platform's own API — real work (S-08 built it from scratch, since the prior project's implementation wasn't available to hand over), but not the differentiator, and everything _upstream_ of publishing is the actual gap. The real product is an editorial judgment, not a scraper: a rule that ranks Spanish real estate stories by how geographically close they are to this audience (Barcelona/Catalonia first, Spain-wide-national next, global-with-Spanish-impact after) and whether they touch money or regulation. Automating that judgment — while keeping the human's two irreducible decisions (what to publish, and final approval) — turns zero posts a week into consistent posts a week at a cost of minutes, not hours.
...
- **A custom social-publishing framework beyond what one platform integration needs** — S-08 built Instagram/Facebook/LinkedIn posting directly against each platform's own API (the prior project's implementation wasn't available to hand over, so "reuse it as-is" was revisited during S-08 planning), but there is still no general multi-tenant publishing abstraction, no support for additional platforms beyond these three, and no token-refresh/rotation system — credentials are operator-managed `.env` values like every other integration in this repo.
